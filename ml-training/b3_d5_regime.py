"""
b3_d5_regime.py — Session B.3 d5 regime-split backtest for 1h.

Same pattern as b2_d3b_regime_backtest.py: fixed train_end=2022-05-01, then
run 3 tier policies on 3 distinct regime windows (bear 2022, chop 2023,
bull 2024 H1). Reuses feature_engine / train_ensemble / backtest_v2 primitives.
"""
import json
import pathlib
import sys
...
def train_model(symbol, interval, train_end):
    ...
    actual_end = max(0, train_idx - embargo)
    X_train = feat_m[FEATURE_NAMES].iloc[:actual_end].to_numpy()
    y_train = target[:actual_end]
    xgb, lgbm, rf, meta = train_ensemble(X_train, y_train, quick=False)
    print(f"  {interval}: trained on {actual_end} rows (embargo {embargo})")
...
def slice_idx(feat, start, end):
    ...
    m = (times >= ts_s) & (times < ts_e)
    return np.where(m.to_numpy())[0]


def run_regime(tier_name, cfg, feat_1h, df_1h, proba_1h, hc, idx,
               daily_proba, daily_dates, fee=0.001, slippage=0.0003):
    closes = feat_1h["close"].iloc[idx].to_numpy().astype(float)
    highs = df_1h["high"].iloc[idx].to_numpy().astype(float)
    lows = df_1h["low"].iloc[idx].to_numpy().astype(float)
    atr_full = _atr(df_1h["high"].to_numpy(),
                    df_1h["low"].to_numpy(),
                    df_1h["close"].to_numpy(), 14)
    atr = atr_full[idx].astype(float)
    dates = pd.to_datetime(feat_1h["open_time"].iloc[idx].values)
    proba = np.asarray(proba_1h[idx], dtype=float)
    feats_test = feat_1h[["rsi_14", "bb_position", "adx_14", "atr_norm_14"]].iloc[idx].reset_index(drop=True)
    ...
    while i < n:
        ...
        passed, reason = apply_gates(
            ...
            hc_threshold=hc, interval="1h",
            label=label, trades_today=trades_today[day_key],
        )
        if not passed:
            rejected[reason] += 1
            i += 1
...
def main():
    ...
    print(f"=== B.3 d5 — 1h regime split backtest ===")
    print(f"Train end: {train_end}")
    for reg, (s, e) in REGIME_WINDOWS.items():
        print(f"  {reg}: {s} .. {e}")
    print()

    print("Training 1h model...")
    xgb, lgbm, rf, meta, feat, df_1h = train_model(SYMBOL, "1h", train_end)
    X_all = feat[FEATURE_NAMES].to_numpy()
    proba_all, _ = ensemble_predict(xgb, lgbm, rf, meta, X_all)
    print(f"1h proba: min={proba_all.min():.3f} max={proba_all.max():.3f} mean={proba_all.mean():.3f}")

    print("Training 1d MTF filter...")
    xgb_d, lgbm_d, rf_d, meta_d, feat_1d, _ = train_model(SYMBOL, "1d", train_end)
    X_1d_all = feat_1d[FEATURE_NAMES].to_numpy()
    proba_1d_all, _ = ensemble_predict(xgb_d, lgbm_d, rf_d, meta_d, X_1d_all)
    daily_dates = pd.to_datetime(feat_1d["open_time"].values)

    thresholds = load_best_thresholds()
    hc = thresholds.get(f"{SYMBOL}_1h", FALLBACK_HC["1h"])
    ...
    for tier_name in ("conservative", "balanced", "aggressive"):
        ...
        for reg, (s, e) in REGIME_WINDOWS.items():
            ...
            if len(idx) == 0:
                continue
            trades, rej = run_regime(tier_name, cfg, feat, df_1h, proba_all, hc, idx,
                                      proba_1d_all, daily_dates)
            ...
            if len(returns) >= 2 and returns.std() > 0:
                sharpe = returns.mean() / returns.std() * np.sqrt(_annual_factor("1h", avg_bars))
            else:
                sharpe = 0.0
    ...
    out = ROOT / "logs" / "b3_d5_regime_summary.json"
