...
| any `macro` object (macro cards; digest/top with a macro winner) | `freshness.captured_at`, `fear_greed.age_hours`, `fear_greed.fetched_at` | the backend's response time, the age counted to it, and the backend's re-request clock for the index (added 2026-09-23: after lamp quotes were bucketed it was the one field left moving the macro hash every few minutes; the index's own time is `as_of`, which stays) |
