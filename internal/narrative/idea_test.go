...
func TestHaikuIdeaGenerator_HappyPath(t *testing.T) {
	...
	const expectedReply = "JTO leads on volume while SOL lags the move. Watch JTO above $2.40 for a confirmation breakout. Risk: a daily close below $2.20 invalidates the long thesis."
	...
	srv := httptest.NewServer(http.HandlerFunc(func(w http.ResponseWriter, r *http.Request) {
		...
		// Body shape check — assert the system prompt and at least the
		// "Narrative:" prefix from the user message reached us. We don't
		// pin the full body byte-for-byte to avoid flakes from formatter
		// drift; the prefix check is a reliable regression signal.
		body, _ := io.ReadAll(r.Body)
		bs := string(body)
		if !strings.Contains(bs, "trader vocabulary") {
			t.Errorf("system prompt missing from request body")
		}
...
// you can update the LLM prompt expectations in lockstep.
func TestHaikuIdeaGenerator_PromptShape(t *testing.T) {
	...
	wantSubstrings := []string{
		...
		"Related assets: FET $1.23 +5.7%, RNDR $8.45 -2.3%",
		"Importance reasons: Only 1 source reporting",
	}
	...
	// Phase 1 LLM-language-mismatch bug.
	if strings.Contains(got, "Нарратив") {
		t.Errorf("user message contains Russian label 'Нарратив'; must be English-only.\nfull message:\n%s", got)
	}
}

// TestHaikuIdeaGenerator_PromptShape_NoPrices verifies the fallback path
// where Binance failed and the prompt has to lean on the bare ticker list
// from the snapshot.
func TestHaikuIdeaGenerator_PromptShape_NoPrices(t *testing.T) {
	...
	// Must NOT contain the formatted-price marker.
...
// TestHaikuIdeaGenerator_StripsMarkdownFence: Haiku occasionally wraps its
// reply in ```...``` even when told not to. cleanIdeaText strips it; this
...
func TestHaikuIdeaGenerator_StripsMarkdownFence(t *testing.T) {
	...
	cases := []struct {
	...
	}{
		{
			...
			raw:  "```\nJTO лидирует. Пробой $2.40.\n```",
			want: "JTO лидирует. Пробой $2.40.",
		},
		{
			name: "fenced with language",
			raw:  "```text\nFET в моменте. Объём растёт.\n```",
			want: "FET в моменте. Объём растёт.",
		},
		{
			name: "no fence — unchanged",
			raw:  "ENA на пробое $0.40 — фикс выше уровня.",
			want: "ENA на пробое $0.40 — фикс выше уровня.",
		},
		{
			name: "trailing whitespace",
			raw:  "  TIA в зоне поддержки.  \n",
			want: "TIA в зоне поддержки.",
		},
	}
